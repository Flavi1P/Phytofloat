...
#		print ox_sat[f][:,1]
	...
	plt.subplot(323)
	plt.plot(chl_cp[f][:,4], -depth, '-')
	plt.title('chl/CP')
	plt.xlim((0,10))

	plt.subplot(324)
	plt.plot(bbp_cp[f][:,4], -depth, '-')
	plt.title('bbp/CP')
	plt.xlim((0,0.1))
	...
	figname = '/var/www/oao/bioargo/PHP/AP/REMBAU/AP_PROF_TXT'+floats[f]+'.png'
...
#	print T
	...
	# Plot Chlorophyll
	ctot_plot = contour_plot_chl(ax[0],T,depth,chl)
...
#	print bact
	...
	# set date format
	date_axis(ax[4],time,months_size=10,fraction=0.04)
	for i in [0,1,2,3,4]:
...
#Save the figurei
	figname = '/var/www/oao/bioargo/PHP/AP/REMBAU/AP'+floats[f]+'.png'
