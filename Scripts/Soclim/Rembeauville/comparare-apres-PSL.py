...
#	chl_spikes[f][chl_spikes[f]>0.05] = np.nan # threshold for acceptable data -> See Nathan for a justification
	...
	ox_sat.append(np.empty(temp[f].shape[0])) ; ox_sat[f] = np.nan
...
#		aou = (O2sol(temp,sal)*dens) - ox
		...
		if any(dep_val):
			...
			mld[p] = 50 # a minmum summer value if daily superficial temperature create a fake MLD (should not be necessary)
